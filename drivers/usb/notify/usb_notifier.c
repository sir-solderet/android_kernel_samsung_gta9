...
extern void update_usb_role(void* pextcon, unsigned int new_role);

void* mtk_get_extcon(void)
{
	struct device_node *np = NULL;
	struct platform_device *pdev = NULL;
	void *extcon = NULL; 
	...
	extcon = platform_get_drvdata(pdev);
err:
	return extcon;
}
#endif
...
static int mtk_set_host(bool enable)
{
	...
	if (enable) {
		...
		usb_role_switch_set_role(get_mtk_role_switch(), USB_ROLE_HOST);
	} else {
		pr_info("%s USB_HOST_DETACHED\n", __func__);
		usb_role_switch_set_role(get_mtk_role_switch(), USB_ROLE_NONE);
	}
#elif IS_ENABLED(CONFIG_USB_MTK_HDRC)
	if (enable) {
		pr_info("%s USB_HOST_ATTACHED\n", __func__);
...
#if defined(CONFIG_CUSTOM_PROJECT_OT11)
	if (enable)
		update_usb_role(mtk_get_extcon(), USB_ROLE_HOST);
	else
		update_usb_role(mtk_get_extcon(), USB_ROLE_NONE);
#endif
	pr_info("%s -\n", __func__);
	return 0;
}
...
#if IS_ENABLED(CONFIG_EXTCON_MTK_USB) && IS_ENABLED(CONFIG_USB_ROLE_SWITCH)
	if (enable) {
		pr_info("%s USB_DEVICE_ATTACHED\n", __func__);
		usb_role_switch_set_role(get_mtk_role_switch(), USB_ROLE_DEVICE);
	} else {
		pr_info("%s USB_DEVICE_DETACHED\n", __func__);
		usb_role_switch_set_role(get_mtk_role_switch(), USB_ROLE_NONE);
	}
#elif IS_ENABLED(CONFIG_USB_MTK_HDRC)
	if (enable) {
...
#if defined(CONFIG_CUSTOM_PROJECT_OT11)
	if (enable)
		update_usb_role(mtk_get_extcon(), USB_ROLE_DEVICE);
	else
		update_usb_role(mtk_get_extcon(), USB_ROLE_NONE);
#endif
	pr_info("%s -\n", __func__);
...
#endif
#if defined(CONFIG_CUSTOM_PROJECT_OT11)
extern void usb_notify_control(bool data_enabled);
#endif
...
#if defined(CONFIG_CUSTOM_PROJECT_OT11)
    .usb_data_enable = usb_notify_control,
#endif
	.vbus_detect_gpio = -1,
	.is_wakelock = 1,
