    ...
    unsigned long value, void *v)
{
    ...
    if (ts && v) {
        if (value == MTK_DISP_EARLY_EVENT_BLANK && *data == MTK_DISP_BLANK_POWERDOWN ) {
            himax_common_suspend(ts->dev);
        } else if (
            #if defined(HX_CONTAINER_SPEED_UP)
            value == MTK_DISP_EARLY_EVENT_BLANK
            #else
            value == MTK_DISP_EVENT_BLANK
            #endif //HX_CONTAINER_SPEED_UP
