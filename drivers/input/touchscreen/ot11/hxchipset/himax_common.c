...
bool himax_earphone_mode_set(bool earphone_sate)
{
    ...
    do {
        if (earphone_sate) {
            himax_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, DATA_LEN_4);
            g_core_fp.fp_register_write(pfw_op->addr_earphone, tmp_data, DATA_LEN_4);
            himax_parse_assign_cmd(fw_func_handshaking_pwd, back_data, DATA_LEN_4);
            I("%s: earphone PLUG_ IN!\n", __func__);
        } else {
            himax_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, DATA_LEN_4);
            g_core_fp.fp_register_write(pfw_op->addr_earphone, tmp_data, DATA_LEN_4);
            himax_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, back_data, DATA_LEN_4);
            I("%s: earphone PLUG_ OUT!\n", __func__);
        }

        g_core_fp.fp_register_read(pfw_op->addr_earphone, tmp_data, DATA_LEN_4);
...
static void himax_earphone_workqueue(struct work_struct *work)
{
    int ret = 0;
    if (private_ts->suspended == false) {
        I("enter headset_workqueue\n");
        ret = himax_earphone_mode_set(private_ts->earphone_enable);
        if (!ret) {
            E("%s:himax headset mode set fail\n", __func__);
        }
    } else {
...
static int himax_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    /*Tab A9 code for AX6739A-1679 by wenghailong at 20230704 start*/
    if (event == EARPHONE_PLUGIN_STATE) {
        private_ts->earphone_enable = 1;
        I("enter earphone in\n");
    } else if (event == EARPHONE_PLUGOUT_STATE) {
        if ((g_tp_typec_earphone_in || g_tp_round_earphone_in) == 0) {
            private_ts->earphone_enable = 0;
            I("enter earphone out\n");
        }
    } else {
        I("enter earphone error\n");
        return -EINVAL;
    }
    if (private_ts->suspended == false) {
        schedule_work(&private_ts->earphone_work_queue);
    } else {
        I("%s:is suspended,do nothing", __func__);
    }
...
static void himax_usb_workqueue(struct work_struct *work)
{
    if (private_ts->suspended == false) {
        I("enter usb_workqueue\n");
...
static int himax_usb_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    ...
    schedule_work(&private_ts->tp_usb_work_queue);
    return 0;
}
#endif // HX_USB_DETECT_GLOBAL
...
static const struct proc_ops_name himax_proc_self_test_ops = {
    owner_line
    .proc_op(open) = himax_self_test_proc_open,
    .proc_op(read) = seq_read,
    .proc_op(write) = himax_self_test_write,
...
static const struct proc_ops_name himax_proc_HSEN_ops = {
    owner_line
    .proc_op(read) = himax_HSEN_read,
    .proc_op(write) = himax_HSEN_write,
};
#endif
...
static const struct proc_ops_name himax_proc_SMWP_ops = {
    owner_line
    .proc_op(read) = himax_SMWP_read,
    .proc_op(write) = himax_SMWP_write,
};
...
static const struct proc_ops_name himax_proc_psensor_ops = {
    owner_line
    .proc_op(read) = himax_psensor_read,
    .proc_op(write) = himax_psensor_write,
};
#endif
#endif
...
static const struct proc_ops_name himax_proc_vendor_ops = {
    owner_line
    .proc_op(read) = himax_vendor_read,
};
...
int himax_input_register(struct himax_ts_data *ts)
{
    ...
    input_set_abs_params(ts->input_dev, ABS_MT_WIDTH_MAJOR,
            ...
            ts->pdata->abs_pressure_fuzz, 0);
#endif
    /*Tab A9 code for SR-AX6739A-01-258 by suyurui at 20230522 start*/
    ...
    #endif //SEC_PALM_FUNC
    /*Tab A9 code for SR-AX6739A-01-258 by suyurui at 20230522 end*/
...
#if defined(HX_ZERO_FLASH)
    /*Tab A9 code for AX6739A-181 by wenghailong at 20230522 start*/
    if (private_ts->in_self_test == 1) {
        I("%s: in self testing, not allow recovery\n", __func__);
    } else {
        /*Tab A9 code for SR-AX6739A-01-282 by wenghailong at 20230502 start*/
        result = g_core_fp.fp_0f_op_file_dirly(private_ts->himax_nomalfw_rq_name);
        /*Tab A9 code for SR-AX6739A-01-282 by wenghailong at 20230502 end*/
        if (result) {
...
int himax_report_data_init(void)
{
    ...
    hx_touch_data->touch_all_size_full_stack = HX_FULL_STACK_RAWDATA_SIZE;
    hx_touch_data->touch_all_size_normal =
        g_core_fp.fp_get_touch_data_size();
    ...
    hx_touch_data->rawdata_size_normal = hx_touch_data->rawdata_size;
    // hx_touch_data->touch_info_size = HX_STACK_ORG_LEN;
    hx_touch_data->rawdata_size =
    ...
    hx_touch_data->rawdata_size_normal -= SEC_FINGER_INFO_SZ;
    #endif //SEC_PALM_FUNC
    /*Tab A9 code for SR-AX6739A-01-258 by suyurui at 20230522 end*/
...
skip_stylus_operation:
    hx_touch_data->touch_all_size = hx_touch_data->touch_all_size_normal;
    hx_touch_data->rawdata_size = hx_touch_data->rawdata_size_normal;
    return NO_ERR;
...
skip_stylus_operation:
    ...
    for (i = 0; i < ts->nFinger_support; i++) {
        ...
        /*Tab A9 code for SR-AX6739A-01-258 by suyurui at 20230522 start*/
        #if defined(SEC_PALM_FUNC)
        maj = hx_touch_data->hx_coord_buf[ts->coordInfoSize+(i*2)];
        min = hx_touch_data->hx_coord_buf[ts->coordInfoSize+(i*2)+1];
        #endif //SEC_PALM_FUNC
        if (g_ts_dbg != 0) {
            #if defined(SEC_PALM_FUNC)
            I("%s: now parsing[%d]:x=%d, y=%d, w=%d, maj=%d, min=%d, palm=%d\n", __func__, i, x, y, w, maj, min, palm);
            #else
        ...
        && y <= ts->pdata->abs_y_max) {
            ...
            /*Tab A9 code for SR-AX6739A-01-258 by suyurui at 20230522 start*/
            #ifdef SEC_PALM_FUNC
            g_target_report_data->maj[i] = maj;
            g_target_report_data->min[i] = min;
            g_target_report_data->palm[i] = palm;
            #endif //SEC_PALM_FUNC
            /*Tab A9 code for SR-AX6739A-01-258 by suyurui at 20230522 end*/
    ...
    case HX_REPORT_COORD_RAWDATA:
        ...
        if (debug_data != NULL) {
            ...
            if (debug_data->is_stack_full_raw) {
                ...
                if (g_ts_dbg != 0)
                    I("%s: event stack raw!\n", __func__);
                if (debug_data->fp_set_diag_cmd(ic_data,
                    hx_touch_data))
                    I("%s:raw data_checksum not match\n",
                        __func__);
            }
...
static void himax_point_report(struct himax_ts_data *ts)
{
    ...
    for (i = 0; i < ts->nFinger_support; i++) {
        ...
        if (valid) {
            ...
            #else
            input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, g_target_report_data->p[i].w);
            #endif //SEC_PALM_FUNC
            /*Tab A9 code for SR-AX6739A-01-258 by suyurui at 20230522 end*/
#if !defined(HX_PROTOCOL_A)
...
static void himax_point_leave(struct himax_ts_data *ts)
{
    ...
    /*    himax_log_touch_event(x, y, w, i, EN_NoiseFilter,*/
    /*            HX_FINGER_LEAVE); */
        ...
        int ts_path, int ts_status)
{
    ...
    memset(ts->xfer_buff,
        0x00,
        hx_touch_data->touch_all_size * sizeof(uint8_t));
...
void himax_enable_gesture_wakeup(void)
{
    struct himax_ts_data *ts = private_ts;
    I("[HIMAX] himax_enable_gesture_wakeup\n");
    ...
    I("[HIMAX] g_smart_wakeup_flag = %d\n", g_smart_wakeup_flag);
}

void himax_disable_gesture_wakeup(void)
{
    struct himax_ts_data *ts = private_ts;
...
static void himax_update_module_info(void)
{
    ...
    {
    case MODEL_GX_HSD:
            strcpy(ts->module_name,"hx83102e_gx_hsd");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_gx_hsd_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_gx_hsd_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx83102e_gx_hsd_criteria.csv");
            break;
    case MODEL_XX_INX:
            strcpy(ts->module_name,"hx83102e_xx_inx");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_xx_inx_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_xx_inx_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx83102e_xx_inx_criteria.csv");
            break;
    case MODEL_LC_HKC:
            strcpy(ts->module_name,"hx83102e_lc_hkc");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_lc_hkc_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_lc_hkc_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx83102e_lc_hkc_criteria.csv");
            break;
    case MODEL_CD_INX:
            strcpy(ts->module_name,"hx83102e_cd_inx");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_cd_inx_firmware.bin");
...
static void tp_feature_interface(void)
{
    ...
    gs_hx_lcm_info->module_name = ts->module_name;
    gs_hx_lcm_info->sec = ts->sec;
    gs_hx_lcm_info->gesture_wakeup_enable = himax_enable_gesture_wakeup;
    gs_hx_lcm_info->gesture_wakeup_disable = himax_disable_gesture_wakeup;
    /*Tab A9 code for AX6739A-241 by suyurui at 20230530 start*/
    gs_hx_lcm_info->fw_version = &(ic_data->vendor_touch_cfg_ver);
    /*Tab A9 code for AX6739A-241 by suyurui at 20230530 end*/
    gs_hx_lcm_info->ito_test = g_core_fp.fp_chip_self_test_for_tp;
    /*Tab A9 code for AX6739A-241 by suyurui at 20230526 start*/
    I("module_name=%s", gs_hx_lcm_info->module_name);
    /*Tab A9 code for AX6739A-241 by suyurui at 20230526 end*/
...
int himax_chip_common_init(void)
{
    ...
    private_ts->chip_max_dsram_size = 0;
 ...
 */
    ...
    /*Tab A9 code for SR-AX6739A-01-265 by wenghailong at 20230510 start*/
    #ifdef CONFIG_ODM_CUSTOM_D85_BUILD
    himax_enable_gesture_wakeup();
    #endif
    /*Tab A9 code for SR-AX6739A-01-265 by wenghailong at 20230510 end*/
    /*Tab A9 code for SR-AX6739A-01-282 by wenghailong at 20230502 start*/
    ...
    /*Tab A9 code for AX6739A-241 by suyurui at 20230526 end*/
    /*Tab A9 code for SR-AX6739A-01-611 by zhawei at 20230516 start*/
    INIT_WORK(&ts->earphone_work_queue, himax_earphone_workqueue);
    ts->earphone_notify.notifier_call = himax_earphone_notifier_callback;
    ret = earphone_notifier_register(&ts->earphone_notify);
    if (ret) {
        /*Tab A9 code for AX6739A-1679 by wenghailong at 20230704 start*/
        g_tp_detect_typec_flag = 0;
        E("Unable to register register earphone notifier: %d\n", ret);
    ...
    /*Tab A9 code for SR-AX6739A-01-263 by zhawei at 20230522 start*/
    #if defined(HX_USB_DETECT_GLOBAL)
    INIT_WORK(&ts->tp_usb_work_queue, himax_usb_workqueue);
    ts->tp_usb_notify.notifier_call = himax_usb_notifier_callback;
    ret = tp_usb_notifier_register(&ts->tp_usb_notify);
    if (ret) {
        g_tp_detect_usb_flag = 0;
        E("Unable to register register usb notifier: %d\n", ret);
        /*Tab A9 code for AX6739A-1679 by wenghailong at 20230704 start*/
        ...
        /*Tab A9 code for SR-AX6739A-01-723 by zhawei at 20230614 start*/
        I("g_tp_detect_usb_flag = 1, start detect usb state\n");
        /*Tab A9 code for SR-AX6739A-01-723 by zhawei at 20230614 end*/
    }
    #endif // HX_USB_DETECT_GLOBAL
    /*Tab A9 code for SR-AX6739A-01-263 by zhawei at 20230522 end*/
...
err_register_usb_notify_failed:
    cancel_work_sync(&ts->tp_usb_work_queue);
    g_tp_detect_typec_flag = 0;
    earphone_notifier_unregister(&ts->tp_usb_notify);
...
void himax_chip_common_deinit(void)
{
    ...
    g_tp_detect_usb_flag = 0;
    cancel_work_sync(&ts->tp_usb_work_queue);
    tp_usb_notifier_unregister(&ts->tp_usb_notify);
    #endif // HX_USB_DETECT_GLOBAL
    /*Tab A9 code for SR-AX6739A-01-263 by zhawei at 20230522 end*/
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
    ...
    himax_report_all_leave_event(ts);
    himax_int_enable(1);
    /*Tab A9 code for AX6739A-170 by zhawei at 20230519 start*/
    himax_earphone_mode_set(ts->earphone_enable);
    /*Tab A9 code for AX6739A-170 by zhawei at 20230519 end*/
    /*Tab A9 code for SR-AX6739A-01-263 by zhawei at 20230522 start*/
    #if defined(HX_USB_DETECT_GLOBAL)
 ...
 */
END:
    /*Tab A9 code for SR-AX6739A-01-662 by zhawei at 20230610 start*/
    if (ic_data != NULL) {
        I("IC_NAME:%s TOUCH_VER:%02x\n", ts->module_name, ic_data->vendor_touch_cfg_ver & 0xFF);
    }
    /*Tab A9 code for SR-AX6739A-01-662 by zhawei at 20230610 end*/
    I("%s: END\n", __func__);
    return 0;
}
