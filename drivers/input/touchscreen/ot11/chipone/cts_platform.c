...
#endif
{
    ...
    /*Tab A9 code for AX6739A-1267 by suyurui at 20230620 start*/
    input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, pdata->res_x-1, 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, pdata->res_y-1, 0, 0);
    /*Tab A9 code for AX6739A-1267 by suyurui at 20230620 end*/
...
int cts_plat_process_touch_msg(struct cts_platform_data *pdata,
        ...
        struct cts_touch_debug_msg *debug_msgs)
{
    struct chipone_ts_data *cts_data;
    ...
    for (i = 0; i < num; i++) {
...
#ifdef SEC_PALM_FUNC
        cts_dbg("  Process touch msg[%d]: id[%u] ev=%u x=%u y=%u p=%u maj=%u min=%u palm=%u",
            i, msgs[i].id, msgs[i].event, x, y, msgs[i].pressure, debug_msgs->major,
            debug_msgs->minor, debug_msgs->palm_flag);
#else
        ...
        /*Tab A9 code for SR-AX6739A-01-667 by suyurui at 20230607 start*/
        ...
        /*Tab A9 code for SR-AX6739A-01-667 by suyurui at 20230607 end*/
...
#ifdef SEC_PALM_FUNC
            input_report_key(input_dev, BTN_PALM, debug_msgs->palm_flag);
            input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR, debug_msgs->major);
            input_report_abs(input_dev, ABS_MT_TOUCH_MINOR, debug_msgs->minor);
#else
    ...
    */
    ...
    /*Tab A9 code for SR-AX6739A-01-655 by suyurui at 20230612 start*/
    if (get_tp_enable()) {
        input_report_key(input_dev, BTN_TOUCH, contact > 0);
    } else {
        cts_info("get_tp_enable() is false. Touch point reporting function turned off\n");
    }
    /*Tab A9 code for SR-AX6739A-01-655 by suyurui at 20230612 end*/
#else
    if (contact == 0) {
