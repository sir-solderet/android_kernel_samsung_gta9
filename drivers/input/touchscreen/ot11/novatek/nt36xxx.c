...
/*Tab A9 code for SR-AX6739A-01-710 by yuli at 20230626 start*/
const struct mtk_chip_config spi_ctrdata = {
    .cs_setuptime = 220,
};
/*Tab A9 code for SR-AX6739A-01-710 by yuli at 20230626 end*/
...
{
    int32_t ret = 0;
    if ((dev == NULL) || (platdata == NULL)) {
        input_err(true, &ts->client->dev, "%s: The parameter dev or platdata is NULL\n");
        return -ENODEV;
    }
#if NVT_TOUCH_SUPPORT_HW_RST
...
void nvt_print_info(void)
{
    ...
    if (ts->touch_count == 0)
        ...
        input_info(true, &ts->client->dev,
                ...
                ts->print_info_cnt_open, ts->print_info_cnt_release);
#elif ODM_CUSTOM_TOUCH_FEATURE
        input_info(true, &ts->client->dev,
                "tc:%d ver:%02d%02d%02d%02d noise:%d lp:%d// #%d %d\n",
                ts->touch_count, ts->fw_ver_bin[0], ts->fw_ver_bin[1],
                ts->fw_ver_bin[2], ts->fw_ver_bin[3], ts->noise_mode, ts->aot_enable,
                ts->print_info_cnt_open, ts->print_info_cnt_release);
#endif
...
*/
    ...
    for (i = 0, finger_cnt = 0; i < platdata->max_touch_num; i++) {
        ...
        if ((status == FINGER_ENTER) || (status == FINGER_MOVING) || (status == GLOVE_TOUCH) || (status == PALM_TOUCH) || (status == METAL_JIG_TOUCH)) {
            ...
            // pressure
            if (i < 2) {
                p = (u32)(p_event_coord->pressure_7_0) + (u32)(point_data[i + 63] << 8);
            ...
            /*Tab A9 code for SR-AX6739A-01-625 by yuli at 20230609 start*/
            // palm & metal_jig should be mutual exist
            input_report_key(ts->input_dev, BTN_PALM, (palm | metal_jig));
            /*Tab A9 code for SR-AX6739A-01-625 by yuli at 20230609 end*/
    ...
    if (get_tp_enable()) {
        ...
        input_info(true, &ts->client->dev, "get_tp_enable() is false. Touch point reporting function turned off\n", __func__);
    }
#else
    input_report_key(ts->input_dev, BTN_TOUCH, (finger_cnt > 0));
...
void nvt_enable_gesture_wakeup(void)
{
    if (!ts->platdata->enable_settings_aot) {
        input_err(true, &ts->client->dev, "aot feature not support %d\n",
...
{
    int ret = 0;
    struct lcm_info *gs_nvt_lcm_info = NULL;

    strcpy(ts->module_name,"nt36523d_lc_hsd");
    gs_nvt_lcm_info = kzalloc(sizeof(*gs_nvt_lcm_info), GFP_KERNEL);
    if (gs_nvt_lcm_info == NULL) {
        ret = -EPERM;
        kfree(gs_nvt_lcm_info);
    }

    gs_nvt_lcm_info->module_name = ts->module_name;
    gs_nvt_lcm_info->fw_version = &(ts->fw_ver_bin[3]);
    gs_nvt_lcm_info->sec = ts->sec;
    gs_nvt_lcm_info->gesture_wakeup_enable = nvt_enable_gesture_wakeup;
    gs_nvt_lcm_info->gesture_wakeup_disable = nvt_disable_gesture_wakeup;

    gs_nvt_lcm_info->ito_test = nvt_selftest_tp_wake_switch;
    tp_common_init(gs_nvt_lcm_info);
    input_info(true, &client->dev, "module_name=%s", gs_nvt_lcm_info->module_name);
...
void nvt_set_usb_mode(int usb_plug_status)
{
    ...
    if (usb_plug_status == USB_DETECT_IN) {
        nvt_set_charger(USB_DETECT_IN);
    } else {
        input_info(true, &client->dev, "Charger flag:%d not currently required!\n", usb_plug_status);
    }
...
static void nvt_usb_notify_work(struct work_struct *work)
{
    if (NULL == work) {
        input_err(true, &client->dev, "parameter work are null!\n");
        return;
    }

    input_info(true, &client->dev, "enter+\n");
    if (mutex_lock_interruptible(&ts->lock)) {
        return;
    }

    if (USB_DETECT_IN == ts->usb_plug_status) {
        input_info(true, &client->dev, "USB plug in");
        nvt_set_charger(USB_DETECT_IN);
    } else if (USB_DETECT_OUT == ts->usb_plug_status) {
        input_info(true, &client->dev, "USB plug out");
    ...
    mutex_unlock(&ts->lock);
    input_info(true, &client->dev, "end-\n");

}

static int nvt_usb_notifier_callback(struct notifier_block *nb, unsigned long event, void *v)
{
    if (ts->fw_update_stat != 1) {
        input_err(true, &client->dev, "Firmware not ready!\n");
        return 0;
    }

    if (event == TP_USB_PLUGIN_STATE) {
        ts->usb_plug_status = 1;
...
static void nvt_earphone_workqueue(struct work_struct *work)
{
    if (ts->earphone_enable) {
        input_info(true, &client->dev, "earphone plug in");
    } else {
        input_info(true, &client->dev, "earphone plug out");
    }

    return;
}

static int nvt_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    if (ts->fw_update_stat != 1) {
        input_err(true, &client->dev, "Firmware not ready!\n");
        return 0;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
    int32_t ret = 0;
    int32_t index = 0;
    struct nvt_ts_platdata *platdata = NULL;
    ...
    /*Tab A9 code for AX6739A-1195 by yuli at 20230615 start*/
    input_info(true, &ts->client->dev, "chip_select=%d\n", ts->client->chip_select);
    ts->client->chip_select = 0;
    /*Tab A9 code for AX6739A-1195 by yuli at 20230615 end*/
    ...
    /*Tab A9 code for AX6739A-1195 by yuli at 20230615 start*/
    input_info(true, &ts->client->dev, "mode=%d, max_speed_hz=%d, cs_id=%d\n", ts->client->mode, ts->client->max_speed_hz, ts->client->chip_select);
    /*Tab A9 code for AX6739A-1195 by yuli at 20230615 end*/
/*
    //---parse dts---
    ret = nvt_parse_dt(&client->dev);
...
*/
    ...
    if (platdata == NULL) {
        input_err(true, &ts->client->dev, "%s: The parameter platdata->irq_gpio is NULL\n");
...
//    if (ts->platdata->support_ear_detect)
#endif
    /*Tab A9 code for AX6739A-1013 by yuli at 20230612 start*/
    device_init_wakeup(&ts->client->dev, 1);
    /*Tab A9 code for AX6739A-1013 by yuli at 20230612 end*/
    ts->platdata->firmware_name = "nt36523d_lc_hsd_firmware.bin";
    /*Tab A9 code for SR-AX6739A-01-706 by yuli at 20230605 start*/
    ts->platdata->firmware_name_mp = "nt36523d_lc_hsd_mpfw.bin";
    /*Tab A9 code for SR-AX6739A-01-706 by yuli at 20230605 end*/
    /*Tab A9 code for SR-AX6739A-01-712 by yuli at 20230625 start*/
    ts->usb_plug_status = UNKOWN_MODE;
    /*Tab A9 code for SR-AX6739A-01-712 by yuli at 20230625 end*/
#if BOOT_UPDATE_FIRMWARE
    ...
    /*Tab A9 code for SR-AX6739A-01-712 by yuli at 20230612 start*/
    INIT_WORK(&ts->tp_usb_work_queue, nvt_usb_notify_work);
    ts->tp_usb_notify.notifier_call = nvt_usb_notifier_callback;
    ret = tp_usb_notifier_register(&ts->tp_usb_notify);
    if (ret) {
        g_tp_detect_usb_flag = 0;
        input_err(true, &client->dev, "Unable to register register usb notifier: %d\n", ret);
    } else {
        g_tp_detect_usb_flag = 1;
    }
    /*Tab A9 code for SR-AX6739A-01-712 by yuli at 20230612 end*/
    /*Tab A9 code for SR-AX6739A-01-621 by yuli at 20230612 start*/
    INIT_WORK(&ts->earphone_work_queue, nvt_earphone_workqueue);
    ts->earphone_notify.notifier_call = nvt_earphone_notifier_callback;
    ret = earphone_notifier_register(&ts->earphone_notify);
    if (ret) {
        /*Tab A9 code for AX6739A-1679 by wenghailong at 20230704 start*/
        g_tp_detect_typec_flag = 0;
        input_err(true, &client->dev,"Unable to register register earphone notifier: %d\n", ret);
    } else {
    ...
    if (active_panel) {
        ...
        if (ret < 0) {
            input_err(true, &ts->client->dev, "register drm_panel_notifier failed. ret=%d\n", ret);
            goto err_register_suspend_resume_failed;
        }
    }
#elif IS_ENABLED(NVT_MSM_DRM_NOTIFY)
    ...
    if (!cookie) {
        input_err(true, &ts->client->dev, "register qcom panel_event_notifier failed\n");
        goto err_register_suspend_resume_failed;
    }
    ...
    if (ret) {
        input_err(true, &ts->client->dev, "register mtk_disp_notifier failed. ret=%d\n", ret);
        goto err_register_suspend_resume_failed;
    }
#endif
    ...
    if (active_panel) {
        if (drm_panel_notifier_unregister(active_panel, &ts->drm_panel_notif))
            input_err(true, &ts->client->dev, "Error occurred while unregistering drm_panel_notifier.\n");
    }
#elif IS_ENABLED(NVT_MSM_DRM_NOTIFY)
...
//    struct nvt_ts_data *ts = dev_get_drvdata(dev);
    ...
    /*Tab A9 code for SR-AX6739A-01-733 by yuli at 20230619 start*/
    #if BOOT_UPDATE_FIRMWARE
    input_info(true, &ts->client->dev, "cancel delayed work fw update sync\n");
    cancel_delayed_work_sync(&ts->nvt_fwu_work);
    ts->isResumeUpdate = false;
...
//    u8 buf[3] = {0};
    ...
    /*Tab A9 code for SR-AX6739A-01-733 by yuli at 20230619 start*/
    #if BOOT_UPDATE_FIRMWARE
    ts->isResumeUpdate = true;
    /*Tab A9 code for AX6739A-1868 by yuli at 20230723 start*/
    queue_delayed_work(nvt_fwu_wq, &ts->nvt_fwu_work, msecs_to_jiffies(60));
    /*Tab A9 code for AX6739A-1868 by yuli at 20230723 end*/
    #else
...
static int nvt_drm_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    ...
    if (!(event == DRM_PANEL_EARLY_EVENT_BLANK ||
        ...
        //input_info(true, &ts->client->dev, "event(%lu) not need to process\n", event);
        return 0;
    }
        ...
        struct panel_event_notification *notification, void *client_data)
{
    ...
    if (!notification) {
        input_err(true, &ts->client->dev, "Invalid notification\n");
        return;
    }

    input_info(true, &ts->client->dev, "Notification type:%d, early_trigger:%d",
    ...
    switch (notification->notif_type) {
        case DRM_PANEL_EVENT_UNBLANK:
            if (notification->notif_data.early_trigger)
                input_info(true, &ts->client->dev, "resume notification pre commit\n");
            else
                nvt_ts_resume(&ts->client->dev);
            break;
        ...
        case DRM_PANEL_EVENT_BLANK:
            if (notification->notif_data.early_trigger)
                ...
                input_info(true, &ts->client->dev, "suspend notification post commit\n");
            break;

        case DRM_PANEL_EVENT_BLANK_LP:
            input_info(true, &ts->client->dev, "received lp event\n");
            break;

        case DRM_PANEL_EVENT_FPS_CHANGE:
            input_info(true, &ts->client->dev, "Received fps change old fps:%d new fps:%d\n",
                    notification->notif_data.old_fps,
                    notification->notif_data.new_fps);
            break;
        default:
            input_info(true, &ts->client->dev, "notification serviced :%d\n",
                    notification->notif_type);
            break;
    }
}
        ...
        unsigned long event, void *data)
{
    ...
    if (!blank) {
        input_err(true, &ts->client->dev, "Invalid blank\n");
        return -1;
    }
    ...
    switch (event) {
        ...
        case MTK_DISP_ESD_RECOVERY_SUSPEND:
        case MTK_DISP_ESD_RECOVERY_RESUME:
        ...
        /*Tab A9 code for AX6739A-1868 by yuli at 20230723 end*/
        default:
            input_info(true, &ts->client->dev, "%s : illegal event!\n", __func__);
            break;
    }
