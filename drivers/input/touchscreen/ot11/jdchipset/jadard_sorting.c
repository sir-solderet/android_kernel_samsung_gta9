...
SAVE_DATA_ERR:
    ...
    return ret_val;
#else
    JD_I("%s: User version does not support this feature\n", __func__);
    return 0;
#endif
}
...
{
    uint8_t os_test_mode_password[JD_TWO_SIZE] = {0};
...
static void jadard_Sorting_BusyStatus_Check(void)
{
    ...
    while (g_module_fp.fp_APP_ReadSortingBusyStatus(JD_MPAP_HANDSHAKE_FINISH, &pStatus) == false) {
        ...
        if (counter++ > 200) {
            Fail_flag = true;
            JD_E("MPAP_HandShake_Fail(0x%02X)\n", pStatus);
            return;
        } else {
            Fail_flag = false;
        }
...
static void jd_test_data_get(uint8_t *RAW, char *start_log, char *result, char *cnt_log, int now_item)
{
    ...
    if (pjadard_ts_data->rawdata_little_endian) {
        max_data = (((int8_t)RAW[index + 1] << 8) | RAW[index]);
        min_data = (((int8_t)RAW[index + 1] << 8) | RAW[index]);
    } else {
        max_data = (((int8_t)RAW[index] << 8) | RAW[index + 1]);
        min_data = (((int8_t)RAW[index] << 8) | RAW[index + 1]);
    }
    ...
    len += scnprintf((testdata + len), SZ_SIZE - len, "\n%s", result);
...
static uint32_t jadard_Item_Test_Function(uint8_t checktype)
{
    ...
    cnt_log = kzalloc(JD_CNT_LOG_LEN * sizeof(char), GFP_KERNEL);
    if (cnt_log == NULL) {
    ...
    scnprintf(cnt_log, JD_CNT_LOG_LEN, "%s%d\n", "Fail_Data_Count: ", fail_data_cnt);
...
static int jadard_parse_sorting_threshold_file(void)
{
    ...
    char *file_name = "jd9366tc_xx_hkc_ito.txt";
    char **result = NULL;
...
static char *get_date_time_str(void)
{
    ...
    timeStart.tv_sec -= (uint64_t)sys_tz.tz_minuteswest * 60;
    rtc_time64_to_tm(timeStart.tv_sec, &rtc_now_time);
...
static int jadard_read_sorting_threshold(void)
{
    ...
    /*Tab A9 code for AX6739A-2005 by hehaoran5 at 20230705 end*/
    jd_g_file_path = vmalloc(JD_RSLT_OUT_FILE_SIZE * sizeof(char));
    if (jd_g_file_path == NULL) {
        JD_E("jd_g_file_path memory alloc fail\n");
        return JD_MEM_ALLOC_FAIL;
    } else {
        memset(jd_g_file_path, 0x00, JD_RSLT_OUT_FILE_SIZE * sizeof(char));
    }
    /*Tab A9 code for AX6739A-2005 by hehaoran5 at 20230705 end*/
...
static int jadard_sorting_test(void)
{
    ...
    uint32_t fw_ret = 0;
    char *fileName = "jd9366tc_xx_hkc_firmware.bin";
    ...
    /*Tab A9 code for AX6739A-1311 by hehaoran5 at 20230619 end*/
#ifdef JD_ZERO_FLASH
    JD_I("%s: Start Bin file name(%s)\n", __func__, fileName);

    fw_ret = g_module_fp.fp_0f_upgrade_fw(fileName);
    if (fw_ret < 0) {
        JD_E("Start Zero flash upgrade fail\n");
        jadard_int_enable(true);
        mutex_unlock(&(pjadard_ts_data->sorting_active));
        pjadard_ts_data->ito_sorting_active = false;
        return JD_SORTING_UPGRADE_FW_ERR;
    } else {
        JD_I("Start Zero flash upgrade success\n");
    }
#endif
    /* END ReCB */
    ...
    scnprintf(jd_g_file_path, JD_RSLT_OUT_FILE_SIZE, "%s%s%s_%s.txt", JD_RSLT_OUT_PATH, JD_RSLT_OUT_FILE, get_date_time_str(), ret == 0 ? "PASS" : "FAIL");
    /*Tab A9 code for SR-AX6739A-01-682 by hehaoran5 at 20230610 end*/
...
END_SORTING_TEST:
    ...
    /*Tab A9 code for AX6739A-1311 by hehaoran5 at 20230619 start*/
    fileName ="jd9366tc_xx_hkc_firmware.bin";
    JD_I("%s: Bin file name(%s)\n", __func__, fileName);
